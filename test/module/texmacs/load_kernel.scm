  ...
  (if (< (length args) 2)
      ...
      (let ((var (car args))
        ...
        (if (or (pair? var) (null? var))
            ...
            (cons 'apply (cons (cons 'lambda (cons var body)) (list val)))
            `(let ((,var ,val)) ,@body)))))
(define-macro (with-global . args)
  `(let ((,(car args) ,(cadr args))) ,@(cddr args)))
(define-macro (and-with . args)
  `(and ,(cadr args) (let ((,(car args) ,(cadr args))) ,@(cddr args))))
(define-macro (with-result . args)
  `(let ((,(car args) '())) ,@(cddr args) (reverse ,(car args))))
(define-macro (repeat . args)
  `(do ((i 0 (+ i 1))) ((= i ,(car args)) (noop)) ,@(cdr args)))
...
;; ----- TeXmacs stubs for kernel files loaded after module restore -----
;; These are normally defined by the init chain, but when we re-load kernel
;; files after set-current-module, the module context may not have them.
(define-macro (tm-define head . body)
  (define (regular? x)
    (not (or (keyword? x)
             (and (pair? x) (keyword? (car x))))))
  (let ((filtered (filter regular? body)))
    (if (pair? head)
        `(define ,(car head) (lambda ,(cdr head) ,@filtered))
        (if (null? filtered) '(begin) `(define ,head ,(car filtered))))))
...
;; Restore module after kernel loading so caller can use its bindings
(set-current-module load-kernel-module)
