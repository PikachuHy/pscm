...
- **控制流**：`if`, `cond`（支持 `else` 和 `=>`）, `case`, `and`, `or`, `begin`
- **作用域**：`let`, `let*`, `letrec`（通过宏展开）
- **循环**：`do`, `for-each`, `map`
- **引用**：`quote`, `quasiquote`
- **函数应用**：`apply`
...
- **异常捕获**：
  ...
  - `catch #t`：捕获所有异常 ✅
- **错误恢复**：
  - `eval_error` 和 `type_error` 使用 `scm_throw` 抛出异常，可被 `catch` 捕获 ✅
  - 未捕获异常由 `scm_uncaught_throw` 处理（打印错误信息后退出）✅

#### 实现细节
- 基于 `setjmp`/`longjmp` 实现非局部跳转
- 使用全局 catch 栈（`g_catch_stack`）管理活跃的 catch
- 标准错误标签为符号 `"error"`（`g_error_key`）
- 错误消息格式化为字符串，包装在 `(message)` 列表中

#### 缺失功能（可选）
- **错误对象**：
  - 错误类型系统（当前使用简单字符串消息）
  - 结构化错误对象（当前使用列表存储错误信息）
...
#### 实现要点
1. 创建 `guile_compat.h` 和 `guile_compat.cc`
2. 类型转换：`SCM*` ↔ `SCM`（void*）
