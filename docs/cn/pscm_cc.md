...
- 支持的数据类型：`NONE`, `NIL`, `LIST`, `PROC`, `CONT`, `FUNC`, `NUM`, `FLOAT`, `CHAR`, `BOOL`, `SYM`, `STR`, `MACRO`, `HASH_TABLE`, `RATIO`
- 每个AST节点都携带可选的源位置信息（文件名、行号、列号），用于错误报告和调试
- 支持整数和浮点数混合运算，自动进行类型提升
- 支持有理数（分数）类型（`RATIO`），参考 guile 1.8 的实现，支持分数的创建、运算和打印（格式为`numerator/denominator`）
...
5. **新特性支持**：
   ...
   - 实现了有理数（分数）类型（`RATIO`），参考 guile 1.8 的实现：
     - 支持分数的创建（`scm_make_ratio`），自动简化（使用GCD）
     - 支持分数的打印（格式为`numerator/denominator`）
     - 支持分数与整数、浮点数的混合运算
     - 除法运算（`/`）自动返回分数类型（如`(/ 1 3)`返回`1/3`）
     - 乘法运算（`*`）支持分数运算（如`(* (/ 1 3) 100)`返回`100/3`）
   - 实现了`expt`函数，支持幂运算（包括负指数）
   - 实现了`number?`函数，用于检查值是否为数字（整数、浮点数或分数）
   - 实现了完整的字符串操作函数集
   - 实现了字符和整数之间的转换
   - 实现了关联列表的完整操作集（增删改查）
